...
fn get( path : PathBuf, query : &str, opts : GetOpts ) -> Result<(), Error>
{
  ...
  if opts.output_toml
  {
    ...
    if opts.format == "yaml"
    {
      let output = serde_yaml::to_string( &JsonItem( item ) )?;
    ...
    match seg
    {
      Num( n ) =>
      {
        let len = match &item
        {
          ...
          _ => return Err( CliError::NotArray().into() ),
        };
        if n >= &len
        {
          return Err( CliError::ArrayIndexOob().into() );
        }
...
impl Serialize for JsonValue<'_>
{
    ...
    S : Serializer,
  {
    ...
    else if self.0.as_date_time().is_some()
    {
